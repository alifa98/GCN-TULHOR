...
class BertSimplePreprocessor(Dataset):
    ...
    def __init__(self, raw_df, should_include_text=False):
        ...
        self.columns = (
            ['masked_sentence', self.MASKED_INDICES_COLUMN, 'sentence', self.TARGET_COLUMN, self.TOKEN_MASK_COLUMN]
            if should_include_text
            else [self.MASKED_INDICES_COLUMN, self.TARGET_COLUMN, self.TOKEN_MASK_COLUMN]
        )
    ...
    def __getitem__(self, idx):
        ...
        return inp.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")), attention_mask.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")), token_mask.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")), mask_target.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    def _prepare_dataset(self):
        ...
        self.optimal_sentence_length = int(np.percentile(exp_lens, self.OPTIMAL_LENGTH_PERCENTILE))
        self._fill_vocab()
    ...
    def _create_item(self, tokens):
        ...
        return (masked, masked_indices, original, original_indices, mask_flags) if self.should_include_text else (masked_indices, original_indices, mask_flags)
    ...
    def _mask_sentence(self, tokens):
        ...
        for _ in range(num_to_mask):
            ...
            if np.random.rand() < 0.8:
                ...
                tokens[idx] = self.vocab_hex.lookup_token(np.random.randint(0, len(self.vocab_hex) - 6))
            mask_flags[idx] = False
        return tokens, mask_flags

    def _pad_sentence(self, tokens, mask_flags):
        if len(tokens) >= self.optimal_sentence_length:
            padded = tokens[:self.optimal_sentence_length]
            padded_mask = [True] + mask_flags[:self.optimal_sentence_length - 1]
        else:
            pad_len = self.optimal_sentence_length - len(tokens)
            padded = tokens + [self.PAD] * pad_len
            padded_mask = [True] + mask_flags + [True] * (pad_len - 1)
...
class BertSimpleTULPreprocessor(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return inp.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")), attention_mask.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")), label.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    def _prepare_dataset(self):
        ...
        processed = [self._create_item(self.tokenizer(seq), uid) for seq, uid in zip(exp, user_ids)]
    ...
    def _pad_sentence(self, tokens):
        return tokens[:self.optimal_sentence_length] + [self.PAD] * max(0, self.optimal_sentence_length - len(tokens))
